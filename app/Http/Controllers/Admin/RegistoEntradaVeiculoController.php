...
class RegistoEntradaVeiculoController extends Controller
{
    ...
    public function store(StoreRegistoEntradaVeiculoRequest $request)
    {
        ...
        return redirect()->route('admin.registo-entrada-veiculos.edit', [$registoEntradaVeiculo->id, 1]);
    }
        ...
        */
        ...
        if (count($registoEntradaVeiculo->frente_parabrisa_photos) > 0) {
            foreach ($registoEntradaVeiculo->frente_parabrisa_photos as $media) {
                if (!in_array($media->file_name, $request->input('frente_parabrisa_photos', []))) {
                    $media->delete();
                }
            }
        }
        $media = $registoEntradaVeiculo->frente_parabrisa_photos->pluck('file_name')->toArray();
        foreach ($request->input('frente_parabrisa_photos', []) as $file) {
            if (count($media) === 0 || !in_array($file, $media)) {
                $registoEntradaVeiculo->addMedia(storage_path('tmp/uploads/' . basename($file)))->toMediaCollection('frente_parabrisa_photos');
            }
        }

        if (count($registoEntradaVeiculo->frente_capo_photos) > 0) {
            foreach ($registoEntradaVeiculo->frente_capo_photos as $media) {
                if (!in_array($media->file_name, $request->input('frente_capo_photos', []))) {
                    $media->delete();
                }
            }
        }
        $media = $registoEntradaVeiculo->frente_capo_photos->pluck('file_name')->toArray();
        foreach ($request->input('frente_capo_photos', []) as $file) {
            if (count($media) === 0 || !in_array($file, $media)) {
                $registoEntradaVeiculo->addMedia(storage_path('tmp/uploads/' . basename($file)))->toMediaCollection('frente_capo_photos');
            }
        }

        if (count($registoEntradaVeiculo->frente_parachoque_photos) > 0) {
            foreach ($registoEntradaVeiculo->frente_parachoque_photos as $media) {
                if (!in_array($media->file_name, $request->input('frente_parachoque_photos', []))) {
                    $media->delete();
                }
            }
        }
        $media = $registoEntradaVeiculo->frente_parachoque_photos->pluck('file_name')->toArray();
        foreach ($request->input('frente_parachoque_photos', []) as $file) {
            if (count($media) === 0 || !in_array($file, $media)) {
                $registoEntradaVeiculo->addMedia(storage_path('tmp/uploads/' . basename($file)))->toMediaCollection('frente_parachoque_photos');
            }
        }
        ...
        if (count($registoEntradaVeiculo->lateral_esquerda_porta_dianteira_photos) > 0) {
            foreach ($registoEntradaVeiculo->lateral_esquerda_porta_dianteira_photos as $media) {
                if (!in_array($media->file_name, $request->input('lateral_esquerda_porta_dianteira_photos', []))) {
                    $media->delete();
                }
            }
        }
        $media = $registoEntradaVeiculo->lateral_esquerda_porta_dianteira_photos->pluck('file_name')->toArray();
        foreach ($request->input('lateral_esquerda_porta_dianteira_photos', []) as $file) {
            if (count($media) === 0 || !in_array($file, $media)) {
                $registoEntradaVeiculo->addMedia(storage_path('tmp/uploads/' . basename($file)))->toMediaCollection('lateral_esquerda_porta_dianteira_photos');
            }
        }

        if (count($registoEntradaVeiculo->lateral_esquerda_porta_traseira_photos) > 0) {
            foreach ($registoEntradaVeiculo->lateral_esquerda_porta_traseira_photos as $media) {
                if (!in_array($media->file_name, $request->input('lateral_esquerda_porta_traseira_photos', []))) {
                    $media->delete();
                }
            }
        }
        $media = $registoEntradaVeiculo->lateral_esquerda_porta_traseira_photos->pluck('file_name')->toArray();
        foreach ($request->input('lateral_esquerda_porta_traseira_photos', []) as $file) {
            if (count($media) === 0 || !in_array($file, $media)) {
                $registoEntradaVeiculo->addMedia(storage_path('tmp/uploads/' . basename($file)))->toMediaCollection('lateral_esquerda_porta_traseira_photos');
            }
        }
        ...
        if (count($registoEntradaVeiculo->cinzeiro) > 0) {
            foreach ($registoEntradaVeiculo->cinzeiro as $media) {
                if (!in_array($media->file_name, $request->input('cinzeiro', []))) {
                    $media->delete();
                }
            }
        }
        $media = $registoEntradaVeiculo->cinzeiro->pluck('file_name')->toArray();
        foreach ($request->input('cinzeiro', []) as $file) {
            if (count($media) === 0 || !in_array($file, $media)) {
                $registoEntradaVeiculo->addMedia(storage_path('tmp/uploads/' . basename($file)))->toMediaCollection('cinzeiro');
            }
        }
        ...
        if (isset($request->save)) {
            ...
            return redirect('/admin/registo-entrada-veiculos/' . $registoEntradaVeiculo->id . '/edit?step=' . $step);
        }
