                        ...
                        <div class="form-group {{ $errors->has('company') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="company_id" id="company_id">
                                @foreach($companies as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('company_id') ? old('company_id') : $driver->company->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('adjustments') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="adjustments[]" id="adjustments" multiple>
                                @foreach($adjustments as $id => $adjustment)
                                    <option value="{{ $id }}" {{ (in_array($id, old('adjustments', [])) || $driver->adjustments->contains($id)) ? 'selected' : '' }}>{{ $adjustment }}</option>
                                @endforeach
                            </select>
