        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.create') }} {{ trans('cruds.drvSegment.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.drv-segments.store") }}" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-group {{ $errors->has('kind') ? 'has-error' : '' }}">
                            ...
                            @foreach(App\Models\DrvSegment::KIND_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="kind_{{ $key }}" name="kind" value="{{ $key }}" {{ old('kind', '') === (string) $key ? 'checked' : '' }} required>
                        ...
                        <div class="form-group {{ $errors->has('duration_seconds') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="duration_seconds" id="duration_seconds" value="{{ old('duration_seconds', '0') }}" step="1" required>
