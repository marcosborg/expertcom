        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.drvSession.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.drv-sessions.update", [$drvSession->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('driver') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="driver_id" id="driver_id" required>
                                @foreach($drivers as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('driver_id') ? old('driver_id') : $drvSession->driver->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('started_at') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control datetime" type="text" name="started_at" id="started_at" value="{{ old('started_at', $drvSession->started_at) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('ended_at') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control datetime" type="text" name="ended_at" id="ended_at" value="{{ old('ended_at', $drvSession->ended_at) }}">
                        ...
                        <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                            ...
                            @foreach(App\Models\DrvSession::STATUS_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $drvSession->status) === (string) $key ? 'checked' : '' }} required>
                        ...
                        <div class="form-group {{ $errors->has('total_drive_seconds') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="total_drive_seconds" id="total_drive_seconds" value="{{ old('total_drive_seconds', $drvSession->total_drive_seconds) }}" step="1" required>
                        ...
                        <div class="form-group {{ $errors->has('total_pause_seconds') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="total_pause_seconds" id="total_pause_seconds" value="{{ old('total_pause_seconds', $drvSession->total_pause_seconds) }}" step="1" required>
                        ...
                        <div class="form-group {{ $errors->has('started_lat') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="started_lat" id="started_lat" value="{{ old('started_lat', $drvSession->started_lat) }}" step="0.0000001">
                        ...
                        <div class="form-group {{ $errors->has('started_lng') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="started_lng" id="started_lng" value="{{ old('started_lng', $drvSession->started_lng) }}" step="0.0000001">
                        ...
                        <div class="form-group {{ $errors->has('ended_lat') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="ended_lat" id="ended_lat" value="{{ old('ended_lat', $drvSession->ended_lat) }}" step="0.0000001">
                        ...
                        <div class="form-group {{ $errors->has('ended_lng') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="ended_lng" id="ended_lng" value="{{ old('ended_lng', $drvSession->ended_lng) }}" step="0.0000001">
                        ...
                        <div class="form-group {{ $errors->has('source') ? 'has-error' : '' }}">
                            ...
                            @foreach(App\Models\DrvSession::SOURCE_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="source_{{ $key }}" name="source" value="{{ $key }}" {{ old('source', $drvSession->source) === (string) $key ? 'checked' : '' }}>
