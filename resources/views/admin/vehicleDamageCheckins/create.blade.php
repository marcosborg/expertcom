        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.create') }} {{ trans('cruds.vehicleDamageCheckin.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.vehicle-damage-checkins.store") }}" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group {{ $errors->has('vehicle_manage_checkin') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="vehicle_manage_checkin_id" id="vehicle_manage_checkin_id" required>
                                @foreach($vehicle_manage_checkins as $id => $entry)
                                    <option value="{{ $id }}" {{ old('vehicle_manage_checkin_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('driver_warning') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="driver_warning" id="driver_warning" value="1" {{ old('driver_warning', 0) == 1 ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('company_warning') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="company_warning" id="company_warning" value="1" {{ old('company_warning', 0) == 1 ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('admin_warning') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="admin_warning" id="admin_warning" value="1" {{ old('admin_warning', 0) == 1 ? 'checked' : '' }}>
